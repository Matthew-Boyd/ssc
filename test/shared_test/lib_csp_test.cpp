...
TEST_F(FlatPlateArrayTest, TestFlatPlateArrayOfOneNominalOperation)
{
    ...
    double useful_power_gain = flat_plate_array_->HeatGainAndLoss(timestamp, external_conditions);  // [W]
    double T_out = flat_plate_array_->HeatFlowsAndOutletTemp(timestamp, external_conditions);                        // [C]
